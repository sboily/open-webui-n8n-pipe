...
class Pipe:
    class Valves(BaseModel):
        ...
        n8n_test_mode: bool = Field(
            default=False, description="Whether to use test mode URLs for n8n webhooks"
        )
        n8n_bearer_token: str = Field(
            default="your-token-here", description="Bearer token for n8n authentication"
        )
        ...
        history_limit: int = Field(
            default=10, description="Maximum number of messages to include in history"
        )
    ...
    def get_webhook_url(self) -> str:
        ...
        Returns:
            Complete webhook URL with appropriate path for test or production mode
        """
        base_url = self.valves.n8n_host.rstrip("/")
        webhook_id = self.valves.n8n_webhook_id
        path = f"/webhook-test/{webhook_id}" if self.valves.n8n_test_mode else f"/webhook/{webhook_id}"
    ...
    def _create_session_id(self, user: Optional[Dict[str, Any]], first_message: Optional[str]) -> str:
    ...
    async def pipe(
        ...
        try:
            ...
            while retry_count <= self.valves.max_retries:
                try:
                    await self.emit_status(
                        __event_emitter__, 
                        "info", 
                        f"Attempt {retry_count + 1}/{self.valves.max_retries + 1}", 
                        False
                    )
            ...
            if self.valves.history_limit > 0 and len(body["messages"]) > self.valves.history_limit:
